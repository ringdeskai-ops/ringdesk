...
module.exports = function(db, sendBrevoEmail) {
  ...
  router.get('/customers', adminRequired, (req, res) => {
    const customers = db.prepare('SELECT id, business_name, email, phone_number, plan, plan_status, ai_name, calls_this_month, call_limit, email_notifications, stripe_customer_id, stripe_subscription_id, created_at, first_name, last_name, contact_phone, address_line1, address_line2, city, county, postcode, country, region FROM clients ORDER BY created_at ASC').all();
  ...
  router.post('/toggle', adminRequired, (req, res) => {
    ...
    const allowed = ['email_notifications', 'voicemail_enabled', 'call_recording'];
