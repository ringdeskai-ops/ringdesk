...
setInterval(() => {
  ...
  for (const [key, val] of oauthStates.entries()) {
    if (now > val.expires) oauthStates.delete(key);
  }
}, 900000);
app.use(express.json({
  verify: (req, res, buf) => {
    ...
    if (req.originalUrl.startsWith('/api/gc/webhook')) {
      req.rawBody = buf;
    }
  }
}));
...
app.post("/api/auth/register", async (req, res) => {
  ...
  const defaultPrompt = `You are the professional AI receptionist for ${business_name}. Warm, natural and concise.
RULES: Max 40 words per response. Natural British English. One question at a time. Never repeat yourself. No markdown or special characters.
ACCENT RULE: Understand English with any accent. Understand slang, broken English and informal speech. Interpret meaning not literal words.
COLLECT IN ORDER: 1. Caller first name. 2. Phone number (combine digits given across multiple responses — only ask to repeat if fewer than 10 digits total, always confirm full number back). 3. Reason for calling.
Once collected say: Thank you, a team member will be in touch shortly.
If asked for pricing say it varies by requirements and offer a free consultation or callback.
TRANSFER: If caller says speak to someone, speak to a person, talk to someone, real person, human, agent, staff, your team, someone else, put me through, or similar — transfer them immediately. Do NOT offer a callback instead.`;
  ...
  try {
    ...
    res.json({ success: true, message: "Registration successful. Please check your email to verify your account.", client_id: id });
  } catch (err) {
...
app.get("/api/client/profile", authRequired, (req, res) => {
  const client = db.prepare("SELECT id, business_name, email, phone_number, plan, plan_status, ai_name, ai_prompt, ai_voice, ai_voice_language, departments, calls_this_month, call_limit, created_at, first_name, last_name, contact_phone, work_phone, mobile_phone, address_line1, address_line2, city, county, postcode, country, region, customer_number, show_demo_banner, feature_email, feature_appointments, feature_ai_settings, feature_voice_selector, feature_crm, voicemail_enabled, call_recording, billing_cycle_day, billing_period_start, signup_completed, cancel_at_period_end, address_type, sms_missed_call, sms_voicemail, sms_after_call, sms_appointment, sms_from_number, gc_mandate_id, gc_subscription_id FROM clients WHERE id = ?").get(req.client.id);
...
app.put("/api/client/settings", authRequired, (req, res) => {
  const { ai_name, ai_prompt, departments, ai_voice, ai_voice_language, work_phone, mobile_phone } = req.body;
  db.prepare("UPDATE clients SET ai_name = ?, ai_prompt = ?, departments = ?, ai_voice = ?, ai_voice_language = ?, work_phone = ?, mobile_phone = ? WHERE id = ?")
    .run(ai_name, ai_prompt, JSON.stringify(departments || {}), ai_voice || 'Google.en-GB-Neural2-C', ai_voice_language || 'en-GB', work_phone || null, mobile_phone || null, req.client.id);
  res.json({ success: true });
});
...
app.post("/api/billing/checkout", authRequired, async (req, res) => {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: `${process.env.DASHBOARD_URL}/?payment=success&session_id={CHECKOUT_SESSION_ID}&client_id=${client.id}`,
...
app.get('/industries', (req, res) => res.sendFile(__dirname + '/public/industries/index.html'));
app.get('/industries/', (req, res) => res.sendFile(__dirname + '/public/industries/index.html'));
app.get('/locations', (req, res) => res.sendFile(__dirname + '/public/locations/index.html'));
...
app.get('/auth/google/login', (req, res) => {
  ...
  const loginState = require('crypto').randomBytes(16).toString('hex');
  // Store state in a simple in-memory map (expires in 10 minutes)
  oauthStates.set(loginState, { type: 'login', expires: Date.now() + 600000 });
...
app.get('/auth/google/login/callback', async (req, res) => {
  ...
  if (!stateData || stateData.type !== 'login' || Date.now() > stateData.expires) {
    return res.redirect('/dashboard?error=google_login_failed');
  }
...
app.get('/auth/google', async (req, res) => {
  ...
  const calState = require('crypto').randomBytes(16).toString('hex');
  // Store state with client ID so we can look it up in callback
  oauthStates.set(calState, { type: 'calendar', clientId: req.client.id, expires: Date.now() + 600000 });
  const url = oauth2Client.generateAuthUrl({
    access_type: 'offline',
    prompt: 'consent',
...
app.get('/auth/google/callback', async (req, res) => {
  ...
  // Verify state to prevent CSRF
  const stateData = oauthStates.get(state);
  if (!stateData || stateData.type !== 'calendar' || Date.now() > stateData.expires) {
  ...
  try {
    ...
    db.prepare("UPDATE clients SET google_access_token = ?, google_refresh_token = ?, google_calendar_connected = 1 WHERE id = ?")
      .run(tokens.access_token, tokens.refresh_token || null, clientId);
...
// ── Check email verified (polling from homepage modal) ────────────────────────
app.post('/api/auth/check-verified', async (req, res) => {
  try {
    const { email } = req.body;
    if (!email) return res.status(400).json({ error: 'Email required' });
    const client = db.prepare('SELECT * FROM clients WHERE email = ?').get(email);
    if (!client) return res.status(404).json({ verified: false });
    if (!client.email_verified) return res.json({ verified: false });
    // Generate token for verified client
    const jwt = require('jsonwebtoken');
    const token = jwt.sign(
      { id: client.id, email: client.email, business_name: client.business_name },
      process.env.JWT_SECRET,
    ...
    res.json({ verified: true, token, client_id: client.id });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/api/auth/resend-verification', async (req, res) => {
  try {
    ...
    const client = db.prepare('SELECT * FROM clients WHERE email = ?').get(email);
    if (!client) return res.status(404).json({ error: 'Not found' });
    if (client.email_verified) return res.json({ success: true, message: 'Already verified' });
    // Generate new token
    const crypto = require('crypto');
    const verToken = crypto.randomBytes(32).toString('hex');
    const verExpires = Math.floor(Date.now() / 1000) + 86400;
    db.prepare('UPDATE clients SET verification_token = ?, verification_expires = ? WHERE id = ?')
      .run(verToken, verExpires, client.id);
    const verifyUrl = process.env.DASHBOARD_URL + '/verify-email?token=' + verToken;
    await sendBrevoEmail(
      email,
      client.business_name || 'there',
      'Verify your AiRingDesk email',
      `<p>Click the link below to verify your email address:</p>
      <p><a href="${verifyUrl}" style="background:#00d4ff;color:#020408;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:700">Verify email →</a></p>
...
// ── GoCardless complete — called after mandate redirect back to homepage ────────
app.post('/api/gc/complete', async (req, res) => {
  try {
    const { client_id, plan } = req.body;
    if (!client_id) return res.status(400).json({ error: 'client_id required' });
    const client = db.prepare('SELECT * FROM clients WHERE id = ?').get(client_id);
    if (!client) return res.status(404).json({ error: 'Client not found' });
    // Complete the GoCardless billing request if pending
    if (client.gc_billing_request_id && !client.gc_mandate_id) {
      try {
        const { GoCardlessClient, Environments } = require('gocardless-nodejs');
        const gcClient = new GoCardlessClient(process.env.GOCARDLESS_ACCESS_TOKEN, Environments.Live);
        const fulfilled = await gcClient.billingRequests.fulfil(client.gc_billing_request_id, {});
        const mandateId = fulfilled.links.mandate;
        const planAmounts = { essential:2900, starter:4900, professional:14900, business:34900 };
        const amount = planAmounts[plan] || planAmounts.essential;
        const subscription = await gcClient.subscriptions.create({
          amount, currency: 'GBP', interval_unit: 'monthly', name: 'AiRingDesk ' + plan,
          links: { mandate: mandateId }
        });
        const PLAN_LIMITS = { essential:150, starter:300, professional:1000, business:5000 };
        const limit = PLAN_LIMITS[plan] || 300;
        db.prepare(`UPDATE clients SET gc_mandate_id=?, gc_subscription_id=?, plan=?, plan_status='active', call_limit=? WHERE id=?`)
          .run(mandateId, subscription.id, plan, limit, client_id);
      } catch(gcErr) {
        console.error('GC complete error:', gcErr.message);
      }
    }
    ...
    localStorage && localStorage.setItem; // noop
    res.json({ success: true, token, user: { id: client.id, email: client.email, business_name: client.business_name, plan } });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.get("/verify-email", (req, res) => {
  ...
  // Generate a temp token so modal can resume at payment step
  const jwtMod = require('jsonwebtoken');
  const verifyToken = jwtMod.sign(
    { id: client.id, email: client.email, business_name: client.business_name },
    process.env.JWT_SECRET,
    { expiresIn: '2h' }
  );
  res.redirect('/?verified=true&client_id=' + client.id + '&token=' + encodeURIComponent(verifyToken) + '&plan=' + (client.plan || 'starter'));
});
    ...
    silenceTimer = setTimeout(async () => {
      ...
      try {
        ...
        if (transferDept) {
          ...
          console.log(`[LIVE] ${callSid} AI speaks: "${spokenReply}"`);
        }

        // Guard: check call is still active before updating
        try {
          const callStatus = await twilio(process.env.TWILIO_ACCOUNT_SID, process.env.TWILIO_AUTH_TOKEN)
            .calls(callSid).fetch();
          if (callStatus.status !== 'in-progress') {
            console.log(`[Deepgram] Call ${callSid} already ended (${callStatus.status}) — skipping reply`);
            processingReply = false;
            return;
          }
        } catch(statusErr) {
          console.log(`[Deepgram] Could not fetch call status for ${callSid} — skipping reply`);
          processingReply = false;
          return;
        }
        ...
        // Silently ignore "call not in progress" errors — caller hung up
        if (err.message && err.message.includes('not in-progress')) {
          console.log(`[Deepgram] Call ${callSid} ended before reply could be sent — ignored`);
        } else {
    ...
    try {
      ...
      if (data.event === 'start') {
        ...
        const keyterms = [
          ...
          'SatFocus','Hikvision','Dahua','Texecom','Pyronix','Ajax','CCTV','intercom','access control','burglar alarm','intruder alarm',
        ...
        dgConnection.on('message', (data) => {
          try {
            ...
            if (!text) return;
            if (!result.is_final) console.log(`[LIVE] ${callSid} caller (partial): "${text}"`);
            if (result.is_final) {
              console.log(`[LIVE] ${callSid} caller (final): "${text}"`);
...
app.post('/api/numbers/provision', authRequired, async (req, res) => {
  ...
  // Payment gate — must have active Stripe or GoCardless subscription
  const hasPaid = client.stripe_subscription_id || client.gc_mandate_id;
  if (!hasPaid) return res.status(402).json({ error: 'Payment required. Please complete checkout before activating your number.' });
  ...
  try {
    ...
    if (!client.ai_prompt) {
      const defaultPrompt = `You are ${client.ai_name || 'Aria'}, the professional AI receptionist for ${client.business_name}. Warm, natural and concise.
RULES: Max 40 words per response. Natural British English. One question at a time. Never repeat yourself. No markdown or special characters.
